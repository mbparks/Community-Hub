...
<header class="site-header">
  ...
  <div class="header-meta">
    ...
    <div class="header-extras">
      ...
      <a class="wave-btn" id="wallLink" href="/wall" style="text-decoration:none;display:inline-block">wall ✏️</a>
...
// ── Wall badge (shows "N new" next to the wall link) ──────────────────────────
async function updateWallBadge() {
  try {
    const r = await fetch('/wall/info');
    const d = await r.json();
    const latest = d.latest || 0;
    const seen = Number(localStorage.getItem('cn_wall_seen') || '0');
    const link = document.getElementById('wallLink');
    if (!link) return;
    if (latest > seen && seen > 0) {
      const n = latest - seen;
      link.innerHTML = 'wall ✏️ <span style="background:var(--c-need-bar);color:white;border-radius:99px;padding:1px 6px;font-size:10px;margin-left:2px">' + n + ' new</span>';
    } else {
      link.textContent = 'wall ✏️';
      if (seen === 0 && latest > 0) {
        // First visit ever; seed the seen-marker so we don't spam "new" on first load
        localStorage.setItem('cn_wall_seen', String(latest));
      }
    }
  } catch (_) {}
}
...
setInterval(updateWallBadge, 30000);  // refresh "N new" wall indicator
...
<div id="panel">
  ...
  <div class="section">
    <div class="section-head">The Wall</div>
    <div class="section-body">
      <div class="row">
        <button class="btn danger" onclick="confirmClearWall()">Clear The Wall</button>
        <a class="btn" href="/wall" target="_blank" style="text-decoration:none;display:inline-block;padding:7px 14px;">Open Wall ↗</a>
      </div>
      <div class="feedback" id="wallFb"></div>
    </div>
  </div>
...
function confirmClearWall() {
  if (!confirm('Wipe the entire wall? Every stroke will be deleted. This cannot be undone.')) return;
  apiFetch(api('/admin/wall/clear'))
    .then(r => r.text()).then(msg => fb('wallFb', '✓ ' + msg))
    .catch(() => fb('wallFb', '✗ Failed'));
}
...
// ============================================================================
// Wall page — shared community graffiti canvas
// ============================================================================
const char WALL_HTML[] PROGMEM = R"PAGE(
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
body {
  ...
  touch-action: none;     /* prevents pinch/scroll from interfering with drawing */
  user-select: none;
...
.top-bar .title { font-size: 15px; font-weight: bold; letter-spacing: 2px; }
.top-bar .back  {
  background: var(--accent-light); color: var(--accent-dark);
  border: 1px solid var(--accent-dark); border-radius: 99px;
  padding: 5px 14px; font-size: 12px; font-weight: bold;
  text-decoration: none; font-family: inherit;
}
.top-bar .stroke-count {
  font-size: 11px; color: var(--accent-light); opacity: 0.8;
}
...
.canvas-wrap {
  ...
  overflow: hidden;
  background:
    repeating-linear-gradient(0deg,  transparent 0 30px, rgba(176,160,128,0.04) 30px 31px),
    repeating-linear-gradient(90deg, transparent 0 30px, rgba(176,160,128,0.04) 30px 31px),
    var(--paper);
}
#wallSvg {
...
@media (max-width: 540px) {
  ...
  .top-bar .title { font-size: 13px; letter-spacing: 1px; }
  .tool-bar      { padding: 8px; gap: 8px; }
  .tool-label    { display: none; }
  .color         { width: 26px; height: 26px; }
  .brush         { width: 30px; height: 30px; }
}
</style>
</head>
<body>
...
<div class="top-bar">
  ...
  <div class="title">✏️ THE WALL</div>
  <div class="stroke-count" id="strokeCount">—</div>
</div>

<div class="canvas-wrap">
  <svg id="wallSvg" viewBox="0 0 1000 600" preserveAspectRatio="xMidYMid meet"></svg>
</div>

<div class="tool-bar">
  <div class="tool-group">
    <span class="tool-label">Color</span>
    <div id="colorRow" style="display:flex;gap:5px"></div>
  </div>
  <div class="tool-group">
    <span class="tool-label">Brush</span>
    <div id="brushRow" style="display:flex;gap:5px"></div>
  </div>
  <span class="spacer"></span>
  <div class="feedback" id="fb"></div>
  <button class="btn subtle" id="undoBtn" onclick="undoMyLast()" disabled>Undo Mine</button>
</div>

<script>
// Graffiti palette — bolder than the board palette, indices match server-side.
// 12 colors, indices 0..11.
const WALL_COLORS = [
  '#222222',  // 0 ink black
  '#e63946',  // 1 vermillion
  '#f4a261',  // 2 tangerine
  '#fbd44b',  // 3 lemon
  '#5fb15a',  // 4 grass green
  '#2a9d8f',  // 5 teal
  '#3a86ff',  // 6 sky blue
  '#4361ee',  // 7 cobalt
  '#9d4edd',  // 8 violet
  '#e85d96',  // 9 magenta
  '#a36e3b',  // 10 sienna
  '#f8f0e0'   // 11 cream (eraser-ish, only visible on the paper grid)
];

// Brush widths, indices 0..3
const WALL_BRUSHES = [2, 4, 8, 14];

const VW = 1000, VH = 600;
const FADE_START_SECS = 7  * 86400;
const FADE_END_SECS   = 14 * 86400;

let myColor = Number(localStorage.getItem('cn_wall_color') || '0');
let myBrush = Number(localStorage.getItem('cn_wall_brush') || '1');
if (myColor < 0 || myColor > 11) myColor = 0;
if (myBrush < 0 || myBrush > 3)  myBrush = 1;

let strokes = [];           // server-known strokes, with {id, t, c, w, p}
let myStrokeIds = [];       // ids of strokes I made this session (for undo)
let lastSeenId = 0;         // for incremental polling

// Mark the wall as seen by stamping localStorage with the latest id
function markWallSeen(id) {
  if (id > 0) localStorage.setItem('cn_wall_seen', String(id));
}
...
function opacityForAge(epochSecs) {
  ...
  const span = FADE_END_SECS - FADE_START_SECS;
  const t = (age - FADE_START_SECS) / span;
  return 1.0 - (t * 0.8);
}

// ── Build a polyline path "d" attribute from flat [x,y,x,y,...] array ───────
...
function renderAll() {
  ...
  // Build all paths in a single innerHTML write for speed
  const parts = strokes.map(s => {
    const op = opacityForAge(s.t);
    if (op <= 0) return '';
    const stroke = WALL_COLORS[s.c] || WALL_COLORS[0];
    const width  = WALL_BRUSHES[s.w] || WALL_BRUSHES[1];
    return `<path d="${pathD(s.p)}" fill="none" stroke="${stroke}"
            stroke-width="${width}" stroke-linecap="round" stroke-linejoin="round"
            opacity="${op.toFixed(3)}" data-id="${s.id}"/>`;
  });
  svg.innerHTML = parts.join('');
  document.getElementById('strokeCount').textContent =
    strokes.length + ' stroke' + (strokes.length !== 1 ? 's' : '');
  const lastId = strokes.length ? strokes[strokes.length - 1].id : 0;
  if (lastId > lastSeenId) lastSeenId = lastId;
  markWallSeen(lastSeenId);
}

function appendStroke(s) {
  // Optimistic local add (used after we POST our own stroke)
  strokes.push(s);
  const svg = document.getElementById('wallSvg');
  const ns = 'http://www.w3.org/2000/svg';
  const p = document.createElementNS(ns, 'path');
  p.setAttribute('d',            pathD(s.p));
  p.setAttribute('fill',         'none');
  p.setAttribute('stroke',       WALL_COLORS[s.c] || WALL_COLORS[0]);
  p.setAttribute('stroke-width', WALL_BRUSHES[s.w] || WALL_BRUSHES[1]);
  p.setAttribute('stroke-linecap',  'round');
  p.setAttribute('stroke-linejoin', 'round');
  p.setAttribute('opacity',      '1');
  p.setAttribute('data-id',      String(s.id));
  svg.appendChild(p);
  document.getElementById('strokeCount').textContent =
    strokes.length + ' stroke' + (strokes.length !== 1 ? 's' : '');
}
...
function buildToolbar() {
  ...
  WALL_COLORS.forEach((col, i) => {
    const el = document.createElement('div');
    el.className = 'color' + (i === myColor ? ' active' : '');
    el.style.background = col;
    el.title = 'color ' + i;
    el.onclick = () => {
      myColor = i;
      localStorage.setItem('cn_wall_color', String(i));
      cRow.querySelectorAll('.color').forEach(c => c.classList.remove('active'));
      el.classList.add('active');
    };
    cRow.appendChild(el);
  });
  const bRow = document.getElementById('brushRow');
  WALL_BRUSHES.forEach((w, i) => {
    const el = document.createElement('div');
    el.className = 'brush' + (i === myBrush ? ' active' : '');
    const dot = document.createElement('div');
    dot.className = 'dot';
    const dotSize = Math.min(22, w * 1.5 + 2);
    dot.style.width  = dotSize + 'px';
    dot.style.height = dotSize + 'px';
    el.appendChild(dot);
    el.onclick = () => {
      myBrush = i;
      localStorage.setItem('cn_wall_brush', String(i));
      bRow.querySelectorAll('.brush').forEach(b => b.classList.remove('active'));
      el.classList.add('active');
    };
    bRow.appendChild(el);
  });
...
const svg = document.getElementById('wallSvg');
let drawing = false;
let currentPoints = [];        // flat [x, y, x, y, ...]
let currentPath = null;        // live SVG element while drawing

function svgPoint(evt) {
  const rect = svg.getBoundingClientRect();
  // Map client coords to viewBox coords (1000 x 600)
...
function startStroke(evt) {
  ...
  currentPath.setAttribute('stroke',       WALL_COLORS[myColor]);
  currentPath.setAttribute('stroke-width', WALL_BRUSHES[myBrush]);
...
function moveStroke(evt) {
  ...
  evt.preventDefault();
  const [x, y] = svgPoint(evt);
  // Skip points that haven't moved enough (cuts down on point count)
  const lastX = currentPoints[currentPoints.length - 2];
  const lastY = currentPoints[currentPoints.length - 1];
  const dx = x - lastX, dy = y - lastY;
  if (dx * dx + dy * dy < 9) return;  // less than 3 viewBox units
  currentPoints.push(x, y);
...
async function endStroke(evt) {
  ...
  if (currentPoints.length < 4) {
    // Single tap with no movement — leave a small dot by duplicating the point
    if (currentPath) currentPath.remove();
    currentPath = null;
    currentPoints = [];
    return;
  }
  // Cap to 60 points (server enforces this too)
  if (currentPoints.length > 120) {
    currentPoints = currentPoints.slice(0, 120);
  }

  // Remove the live preview path; we'll re-add via appendStroke after the server confirms
  if (currentPath) currentPath.remove();
  currentPath = null;

  const payload = { color: myColor, width: myBrush, points: currentPoints };
  try {
    const r = await fetch('/wall/stroke', {
      method: 'POST',
...
svg.addEventListener('pointerdown', startStroke);
...
svg.addEventListener('pointerleave', e => { if (drawing) endStroke(e); });

// ── Undo ────────────────────────────────────────────────────────────────────
// Removes my last stroke from the client view, but does NOT delete it from the
// server. The server has no per-stroke deletion endpoint (it's communal).
// Undo just hides it locally; on next refresh it comes back.
function undoMyLast() {
  if (myStrokeIds.length === 0) return;
  const id = myStrokeIds.pop();
  const el = svg.querySelector(`[data-id="${id}"]`);
  if (el) el.remove();
  // Remove from local strokes array too
  ...
  fb('Hidden locally. Refresh to bring it back.');
}

// ── Polling for new strokes from other clients ──────────────────────────────
...
async function pollNew() {
  ...
  try {
    ...
    if (lastFresh !== lastSeenId || fresh.length !== strokes.length) {
      strokes = fresh;
      renderAll();
    }
  } catch (_) {}
...
function fb(msg) {
  ...
  el.textContent = msg;
  clearTimeout(fb._t);
  fb._t = setTimeout(() => { el.textContent = ''; }, 3500);
}
...
// Re-render every minute too, so fade opacity updates without a refresh
